...
# Plot the numbers under def when plotting, get the sum of only those, not the entire project. 
# Double the while loop number when doing the count for insertion sort.
...
def selection_sort(arr):
  for k in range(len(arr)):
      ...
      # swapping the current k index with the lowest number left
      hold = arr[minloc]
      arr[minloc] = arr[k]
      arr[k] = hold
